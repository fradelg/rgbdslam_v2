...
inline void setGLColor(const PointType& p)
{
  ...
  if (p.segment == 0 || ParameterServer::instance()->get<int>("segment_to_optimize") < 0) { //No segment: original color
    ...
    if (p.segment == ParameterServer::instance()->get<int>("segment_to_optimize")) {
      r = 0;
      g = 255;
      b = 0; //right segment: green
    } else {
      r = 255;
      g = 0;
      b = 0; //wrong segment: red
    }
    ...
    case 0:
      ...
      g = 0;
      b = 0;
      ROS_INFO_COND(p.segment != previous_segment, "%d is black      ", p.segment);
      break; //black
    case 1:
      r = 128;
      g = 128;
      b = 0;
      ROS_INFO_COND(p.segment != previous_segment, "%d is yellow     ", p.segment);
      break; //yellow
    case 2:
      r = 128;
      g = 0;
      b = 0;
      ROS_INFO_COND(p.segment != previous_segment, "%d is red        ", p.segment);
      break; //red
    case 3:
      r = 128;
      g = 0;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is magenta    ", p.segment);
      break; //magenta
    case 4:
      r = 0;
      g = 128;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is cyan       ", p.segment);
      break; //cyan
    case 5:
      r = 0;
      g = 128;
      b = 0;
      ROS_INFO_COND(p.segment != previous_segment, "%d is green      ", p.segment);
      break; //green
    case 6:
      r = 0;
      g = 0;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is blue       ", p.segment);
      break; //blue
    case 7:
      r = 255;
      g = 255;
      b = 255;
      ROS_INFO_COND(p.segment != previous_segment, "%d is white      ", p.segment);
      break; //white
    case 8:
      r = 128;
      g = 128;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is gray       ", p.segment);
      break; //gray
    case 9:
      r = 255;
      g = 255;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is yellowish  ", p.segment);
      break; //yellowish
    case 10:
      r = 255;
      g = 128;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is redish     ", p.segment);
      break; //redish
    case 11:
      r = 255;
      g = 128;
      b = 255;
      ROS_INFO_COND(p.segment != previous_segment, "%d is magentaish ", p.segment);
      break; //magentaish
    case 12:
      r = 128;
      g = 255;
      b = 255;
      ROS_INFO_COND(p.segment != previous_segment, "%d is cyanish    ", p.segment);
      break; //cyanish
    case 13:
      r = 128;
      g = 255;
      b = 128;
      ROS_INFO_COND(p.segment != previous_segment, "%d is greenish   ", p.segment);
      break; //greenish
    case 14:
      r = 128;
      g = 128;
      b = 255;
      ROS_INFO_COND(p.segment != previous_segment, "%d is blueish    ", p.segment);
      break; //blueish
    case 15:
      r = 255;
      g = 255;
      ...
      break; //white
    default:
      ROS_ERROR("Impossible value %d", p.segment % 8);
    }
...
{
  glPushMatrix();
  glMultMatrixf((*cloud_matrices)[i].data()); //works as long as qreal and GLdouble are typedefs to double (might depend on hardware)
  if (show_clouds_)
...
void GLViewer::drawClouds(float xshift)
{
  ...
  glMultMatrixf(viewpoint_tf_.data());
  if (show_grid_) {
    drawGrid(); //Draw a 10x10 grid with 1m x 1m cells
  }
  if (show_poses_)
    drawAxes(0.5); //Show origin as big axis
  ...
  for (int i = 0; i < cloud_list_indices.size() && i < cloud_matrices->size(); i++) {
    glPushMatrix();
    glMultMatrixf((*cloud_matrices)[i].data());
    if (show_poses_)
      drawAxes((i + 1 == cloud_list_indices.size()) ? 0.5 : 0.075); //Draw last pose Big
    if (show_ids_) {
      glColor4f(1 - bg_col_[0], 1 - bg_col_[1], 1 - bg_col_[2], 1.0); //inverse of bg color
...
{ //TODO: consolidate setRotation methods
    ...
    for (int x = 0; x < w - step; x += step) { //for it and its neighbours right and/or down
      ...
      if (!strip_on) { //Generate vertices for new triangle
        ...
        if (!validXYZ(*ll, max_depth))
          continue; // both new triangles in this step would use this point
        const point_type* ur = &pc->points[(x + step) + y * w]; //one right (upper right corner)
        if (!validXYZ(*ur, max_depth))
          continue; // both new triangles in this step would use this point
        ...
        if (validXYZ(*ul, max_depth)) { //ul, ur, ll all valid
          ...
          if (squaredEuclideanDistance(*ul, *ll) / depth <= mesh_thresh and squaredEuclideanDistance(*ul, *ll) / depth <= mesh_thresh and squaredEuclideanDistance(*ur, *ll) / depth <= mesh_thresh) {
        ...
        if (!strip_on) { //can't use the point on the upper left, should I still init a triangle?
          ...
          if (!validXYZ(*lr, max_depth)) {
            ...
            if (squaredEuclideanDistance(*ur, *ll) / depth <= mesh_thresh and squaredEuclideanDistance(*lr, *ll) / depth <= mesh_thresh and squaredEuclideanDistance(*ur, *lr) / depth <= mesh_thresh) {
              glBegin(GL_TRIANGLE_STRIP);
              strip_on = true;
      ...
      else { //neighbours to the left and left down are already set
        const point_type* ul;
        if (flip) {
          ul = &pc->points[(x) + (y + step) * w];
        } //one down (lower left corner)
        else {
          ul = &pc->points[x + y * w];
        } //current point (upper right)
        ...
        if (flip) {
          ...
          ll = &pc->points[(x) + (y + step) * w];
        } //one down (lower left corner)
        if (validXYZ(*ll, max_depth)) {
          depth = squaredEuclideanDistance(*ll, origin);
          if (squaredEuclideanDistance(*ul, *ll) / depth > mesh_thresh or squaredEuclideanDistance(*ul, *(ul - step)) / depth > mesh_thresh or squaredEuclideanDistance(*ll, *(ll - step)) / depth > mesh_thresh) {
            glEnd();
            strip_on = false;
            continue;
          }
...
void GLViewer::drawEdges()
{
  ...
  for (int i = 0; i < edge_list_.size(); i++) {
    ...
    //This happens if the edges are updated faster than the poses/clouds
      ...
      ROS_WARN("Not enough cloud matrices (%d) for vertex ids (%d and %d)", cloud_matrices->size(), id1, id2);
  }
  glEnd();
  //glDisable (GL_LINE_STIPPLE);
...
void GLViewer::drawToPS(QString filename)
{
  ...
  while (state == GL2PS_OVERFLOW) {
    ...
    gl2psBeginPage("GL View", "RGBD-SLAM", viewport,
                   GL2PS_PDF, GL2PS_BSP_SORT, GL2PS_SILENT | GL2PS_SIMPLE_LINE_OFFSET | GL2PS_NO_BLENDING | GL2PS_OCCLUSION_CULL | GL2PS_BEST_ROOT,
