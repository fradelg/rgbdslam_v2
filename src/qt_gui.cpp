...
void Graphical_UI::setup()
{
  ...
  QString vl("Waiting for visual image on topic<br/><i>\"");
  vl += visual_topic.c_str();
  vl += "\"</i>";
  ...
  QString dl("Waiting for depth image on topic<br/><i>\"");
  dl += depth_topic.c_str();
  dl += "\"</i><br/>";
...
void Graphical_UI::setRotationGrid()
{
  ...
  if (ok) {
    glviewer->setRotationGrid(value);
  }
}
...
QAction* Graphical_UI::createSaveMenu()
{ //octomap Menu
