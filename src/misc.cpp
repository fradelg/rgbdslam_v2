...
{
  Eigen::Matrix<double, 4, 4> m = se3.to_homogeneous_matrix();
  ROS_DEBUG_STREAM("Eigen Matrix:\n"
                   << m);
  QMatrix4x4 qmat((float*)m.data(), 4, 4);
...
bool isBigTrafo(const Eigen::Isometry3d& t)
{
  ...
  return (dist > ParameterServer::instance()->get<double>("min_translation_meter") || angle > ParameterServer::instance()->get<double>("min_rotation_degree"));
}
...
bool isSmallTrafo(const Eigen::Isometry3d& t, double seconds)
{
  ...
  ParameterServer* ps = ParameterServer::instance();
  return (dist / seconds < ps->get<double>("max_translation_meter") && angle_around_axis / seconds < ps->get<double>("max_rotation_degree"));
}
...
bool isBigTrafo(const g2o::SE3Quat& t)
{
  ...
  //Q_EMIT setGUIInfo2(infostring);
  ParameterServer* ps = ParameterServer::instance();
  return (dist > ps->get<double>("min_translation_meter") || angle_around_axis > ps->get<double>("min_rotation_degree"));
}
...
std::string openCVCode2String(unsigned int code)
{
  switch (code) {
  case 0:
    return std::string("CV_8UC1");
  case 8:
    return std::string("CV_8UC2");
  case 16:
    return std::string("CV_8UC3");
  case 24:
    return std::string("CV_8UC4");
  case 2:
    return std::string("CV_16UC1");
  case 10:
    return std::string("CV_16UC2");
  case 18:
    return std::string("CV_16UC3");
  case 26:
    return std::string("CV_16UC4");
  case 5:
    return std::string("CV_32FC1");
  case 13:
    return std::string("CV_32FC2");
  case 21:
    return std::string("CV_32FC3");
  case 29:
    return std::string("CV_32FC4");
  }
                                        ...
                                        const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  ...
  if (rgb_img.type() == CV_8UC1)
    pixel_data_size = 1;
  else if (ps->get<bool>("encoding_bgr")) {
    red_idx = 2;
    blue_idx = 0;
  }
                                        ...
                                        const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  ...
  int pixel_data_size = 3;
  char red_idx = 0, green_idx = 1, blue_idx = 2;
  if (rgb_msg->encoding.compare("mono8") == 0)
    pixel_data_size = 1;
...
{
  //FIXME: Take from parameter_server or cam info
...
float getMinDepthInNeighborhood(const cv::Mat& depth, cv::Point2f center, float diameter)
{
  ...
  bot = bot > depth.rows ? depth.rows : bot;
  int right = center.x + radius;
  right = right > depth.cols ? depth.cols : right;
                           ...
                           unsigned int& all)
{
  ...
  for (int new_ry = 0; new_ry < (int)new_pc->height; new_ry += skip_step) {
    for (int new_rx = 0; new_rx < (int)new_pc->width; new_rx += skip_step, all++) {
      ...
      if (old_rx_center >= (int)old_pc->width || old_rx_center < 0 || old_ry_center >= (int)old_pc->height || old_ry_center < 0) {
      ...
      for (int old_ry = starty; old_ry < endy; old_ry += neighbourhood_step) {
        for (int old_rx = startx; old_rx < endx; old_rx += neighbourhood_step) {
          ...
          if (p_new_in_front < 0.001) { // it is in behind and outside the 99.8% interval
            occluded_point = true; //Outside, but occluded
          } else if (p_new_in_front < 0.999) { // it is inside the 99.8% interval (and not behind)
            good_point = true;
