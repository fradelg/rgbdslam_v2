...
QList<int> GraphManager::getPotentialEdgeTargetsWithDijkstra(const Node* new_node, int sequential_targets, int geodesic_targets, int sampled_targets, int predecessor_id, bool include_predecessor)
{
  ...
  if ((int)camera_vertices.size() <= sequential_targets + geodesic_targets + sampled_targets || camera_vertices.size() <= 1) { //if less prev nodes available than targets requestet, just use all
                                   ...
                                   bool& edge_to_keyframe) ///Output:contains the best-yet of the pairwise motion estimates for the current node
{
  ...
  if (num_keypoints < ps->get<int>("min_matches") && !ps->get<bool>("keep_all_nodes")) {
  ...
  if (ps->get<double>("min_translation_meter") > 0.0 || ps->get<double>("min_rotation_degree") > 0.0) {
    //First check if trafo to last frame is big
    //Node* prev_frame = graph_[graph_.size()-1];
    Node* prev_frame = graph_[graph_.size() - 1];
    if (localization_only_ && curr_best_result_.edge.id1 > 0) {
      prev_frame = graph_[curr_best_result_.edge.id1];
    }
  ...
  if (ps->get<bool>("concurrent_edge_construction")) {
    ...
    for (int i = 0; i < results.size(); i++) {
      ...
      if (mr.edge.id1 >= 0) {
        ...
        if (isSmallTrafo(mr.edge.transform, delta_time.toSec()) && addEdgeToG2O(mr.edge, graph_[mr.edge.id1], new_node, isBigTrafo(mr.edge.transform), mr.inlier_matches.size() > curr_best_result_.inlier_matches.size(), curr_motion_estimate)) {
          graph_[new_node->id_] = new_node; //Needs to be added
    ...
    for (int id_of_id = (int)vertices_to_comp.size() - 1; id_of_id >= 0; id_of_id--) {
      ...
      if (mr.edge.id1 >= 0) {
        ros::Duration delta_time = new_node->header_.stamp - graph_[mr.edge.id1]->header_.stamp;
        if (isSmallTrafo(mr.edge.transform, delta_time.toSec()) && addEdgeToG2O(mr.edge, node_to_compare, new_node, isBigTrafo(mr.edge.transform), mr.inlier_matches.size() > curr_best_result_.inlier_matches.size(), curr_motion_estimate)) {
#ifdef DO_FEATURE_OPTIMIZATION
  ...
  bool keep_anyway = (ps->get<bool>("keep_all_nodes") || (((int)new_node->feature_locations_3d_.size() > ps->get<int>("min_matches"))
  ...
  if ((!found_trafo && valid_odometry) || ((!found_trafo && keep_anyway) || (!predecessor_matched && time_delta_sec < 0.1))) //FIXME: Add parameter for constant position assumption and time_delta
  {
...
bool GraphManager::addNode(Node* new_node)
{
  ...
  if (found_match) { //Success
    ...
    } else //Mapping
    {
      ...
      if (ps->get<int>("optimizer_skip_step") > 0 && (camera_vertices.size() % ps->get<int>("optimizer_skip_step")) == 0) {
        optimizeGraph();
      }
...
void GraphManager::addKeyframe(int id)
{
  ...
  std::stringstream ss;
  ss << keyframe_ids_.size() << " Keyframes: ";
  BOOST_FOREACH (int i, keyframe_ids_) {
    ss << i << ", ";
  }
  ROS_INFO("%s", ss.str().c_str());
}
...
QList<QPair<int, int> >* GraphManager::getGraphEdges()
{
  ...
  for (; edge_iter != odom_and_feature_edges.end(); edge_iter++) {
    ...
    if (node_id1_it == vertex_id_to_node_id.end())
      continue;
    std::map<int, int>::iterator node_id2_it = vertex_id_to_node_id.find(v2->id());
    if (node_id2_it == vertex_id_to_node_id.end())
      continue;
