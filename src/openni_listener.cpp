  ...
  { //bag will be destructed after this block (hopefully frees memory for the optimizer)
    ...
    BOOST_FOREACH (rosbag::MessageInstance const m, view) {
      ...
      if (m.getTopic() == odom_tpc || ("/" + m.getTopic() == odom_tpc)) {
        ...
        if (depth_img)
          dep_images.push_back(depth_img);
                                    ...
                                    const sensor_msgs::PointCloud2ConstPtr& point_cloud)
{
  ...
  if (visual_img.rows != depth_img.rows || visual_img.cols != depth_img.cols || point_cloud->width != (uint32_t)visual_img.cols || point_cloud->height != (uint32_t)visual_img.rows) {
                                    ...
                                    const sensor_msgs::PointCloud2ConstPtr& point_cloud)
{
  ...
  cv::Mat depth_float_img = cv_bridge::toCvCopy(depth_img_msg)->image;
  cv::Mat visual_img = cv_bridge::toCvCopy(visual_img_msg)->image;
  if (visual_img.rows != depth_float_img.rows || visual_img.cols != depth_float_img.cols || point_cloud->width != (uint32_t)visual_img.cols || point_cloud->height != (uint32_t)visual_img.rows) {
    ROS_ERROR("PointCloud, depth and visual image differ in size! Ignoring Data");
    return;
  }
  image_encoding_ = visual_img_msg->encoding;
                                    ...
                                    cv::Mat depth_mono8_img)
{
  ...
  if (getOneFrame_) {
    getOneFrame_ = false;
  } //if getOneFrame_ is set, unset it and skip check for  pause
  else if (pause_) {
    return;
...
void OpenNIListener::togglePause()
{
  ...
  if (pause_)
    Q_EMIT setGUIStatus("Processing Thread Paused");
  else
    Q_EMIT setGUIStatus("Processing Thread Running");
}
...
QImage OpenNIListener::cvMat2QImage(const cv::Mat& channel1, const cv::Mat& channel2, const cv::Mat& channel3, unsigned int idx)
{
  ...
  if (channel2.rows != channel1.rows || channel2.cols != channel1.cols || channel3.rows != channel1.rows || channel3.cols != channel1.cols) {
...
QImage OpenNIListener::cvMat2QImage(const cv::Mat& image, unsigned int idx)
{
  ...
  if (image_encoding_.compare("rgb8") == 0) {
    red_idx = 2;
    blue_idx = 0;
  }
